...
func migrate() error {
	...
	db.Exec(`CREATE TABLE IF NOT EXISTS critic_scores (
		...
		missed_angles INTEGER DEFAULT 0,
		architectural_awareness INTEGER DEFAULT 0,
		actionable_value INTEGER DEFAULT 0,
...
func AddMetaLearning(ml MetaLearning) (int64, error) {
	result, err := db.Exec(
		`INSERT INTO meta_learnings (repo, scope, key, value, source_issue, score_at_creation) VALUES (?, ?, ?, ?, ?, ?)`,
...
func GetMetaLearnings(repo string, limit int) ([]MetaLearning, error) {
	rows, err := db.Query(
		`SELECT id, repo, scope, key, value, source_issue, score_at_creation, times_applied, created_at, updated_at
		 FROM meta_learnings WHERE repo = ? OR scope = 'general' ORDER BY times_applied DESC, id DESC LIMIT ?`,
		repo, limit,
	)
	if err != nil {
		return nil, err
	...
	defer rows.Close()
	var learnings []MetaLearning
	for rows.Next() {
		var ml MetaLearning
		if err := rows.Scan(&ml.ID, &ml.Repo, &ml.Scope, &ml.Key, &ml.Value, &ml.SourceIssue, &ml.ScoreAtCreation, &ml.TimesApplied, &ml.CreatedAt, &ml.UpdatedAt); err != nil {
			return nil, err
		}
		learnings = append(learnings, ml)
	}
	return learnings, rows.Err()
}

func IncrementTimesApplied(ids []int64) {
	for _, id := range ids {
		db.Exec(`UPDATE meta_learnings SET times_applied = times_applied + 1, updated_at = datetime('now') WHERE id = ?`, id)
	}
}
...
func AddCriticScore(cs CriticScore) (int64, error) {
	result, err := db.Exec(
		`INSERT INTO critic_scores (repo, issue_number, task_type, score, depth, missed_angles, architectural_awareness, actionable_value, calibration, strengths, weaknesses)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		cs.Repo, cs.IssueNumber, cs.TaskType, cs.Score, cs.Depth, cs.MissedAngles, cs.ArchitecturalAwareness, cs.ActionableValue, cs.Calibration, cs.Strengths, cs.Weaknesses,
	)
	if err != nil {
		return 0, err
	}
...
func GetCriticScores(repo string, limit int) ([]CriticScore, error) {
	rows, err := db.Query(
		`SELECT id, repo, issue_number, task_type, score, depth, missed_angles, architectural_awareness, actionable_value, calibration, strengths, weaknesses, created_at
	...
	for rows.Next() {
		var cs CriticScore
		if err := rows.Scan(&cs.ID, &cs.Repo, &cs.IssueNumber, &cs.TaskType, &cs.Score, &cs.Depth, &cs.MissedAngles, &cs.ArchitecturalAwareness, &cs.ActionableValue, &cs.Calibration, &cs.Strengths, &cs.Weaknesses, &cs.CreatedAt); err != nil {
			return nil, err
		}
